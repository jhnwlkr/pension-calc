...
case "${1:-all}" in
  ...
  all)
    ...
    query "SELECT blob2 AS country, SUM(_sample_interval) AS clicks, COUNT(DISTINCT blob1) AS unique_users FROM $DATASET WHERE timestamp > NOW() - INTERVAL '7' DAY GROUP BY country ORDER BY clicks DESC"
    echo ""
    echo "=== Last 7 days — Clicks by device ==="
    query "SELECT blob3 AS device, SUM(_sample_interval) AS clicks, COUNT(DISTINCT blob1) AS unique_users FROM $DATASET WHERE timestamp > NOW() - INTERVAL '7' DAY GROUP BY device ORDER BY clicks DESC"
    echo ""
    echo "=== Last 7 days — Clicks by referrer ==="
    query "SELECT blob4 AS referrer, SUM(_sample_interval) AS clicks FROM $DATASET WHERE timestamp > NOW() - INTERVAL '7' DAY GROUP BY referrer ORDER BY clicks DESC"
    echo ""
    echo "=== Last 7 days — Active tab at Calculate ==="
    query "SELECT blob5 AS active_tab, SUM(_sample_interval) AS clicks FROM $DATASET WHERE timestamp > NOW() - INTERVAL '7' DAY GROUP BY active_tab ORDER BY clicks DESC"
    echo ""
